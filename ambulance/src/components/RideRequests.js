...
//         { withCredentials: true }
//       );
//       const acceptedRequest = pendingRequests.find(request => request._id === requestId);
...
// }
import React, { useEffect, useState } from "react";
import "./RideRequests.css"; // Import CSS for styling
import axios from "axios";
...
export default function RideRequests() {
  ...
  const handleAccept = async (requestId) => {
    try {
      ...
      const acceptedRequest = pendingRequests.find((request) => request._id === requestId);
      // Update the state: Remove from pending, add to accepted
      setPendingRequests(pendingRequests.filter((request) => request._id !== requestId));
      setAcceptedRequests([...acceptedRequests, acceptedRequest]); // Add to accepted requests
      alert("Request Accepted!");
    } catch (error) {
      console.error("Error accepting ride request:", error);
    }
  ...
  return (
    <div className="ride-requests-container">
      <div className="ride-column pending-requests">
        <h2 style={{color:"#073057"}}>Pending Ride Requests</h2>
        {pendingRequests.length > 0 ? (
          <ul className="list-group">
            {pendingRequests.map((request, index) => (
              <li
      ...
      <div className="ride-column accepted-requests">
        <h2 style={{color:"#073057"}}>Accepted Ride Requests</h2>
        {acceptedRequests.length > 0 ? (
          <ul className="list-group">
            {acceptedRequests.map((request, index) => (
              <li key={request._id} className="list-group-item d-flex justify-content-between">
